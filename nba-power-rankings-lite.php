<?php
/*
Plugin Name: NBA Basketball Power Rankings Lite
...
function NBASPRLPageURL() {
 $NBASPRLpageURL = 'http';
 $NBASPRLpageURL .= "://";
 if ($_SERVER["SERVER_PORT"] != "80") {
  $NBASPRLpageURL .= $_SERVER["SERVER_NAME"].":".$_SERVER["SERVER_PORT"].$_SERVER["REQUEST_URI"];
 } else {
  $NBASPRLpageURL .= $_SERVER["SERVER_NAME"].$_SERVER["REQUEST_URI"];
 }
 return $NBASPRLpageURL;
}
/* This Registers a Sidebar Widget.*/
function widget_nbasprlstats() 
{
?>
<h2>NBA Power Rankings Lite</h2>
<?php nbasprl_stats(); ?>
<?php
}

function nbasprlstats_install()
{
register_sidebar_widget(__('NBA Power Rankings Lite'), 'widget_nbasprlstats'); 
}
add_action("plugins_loaded", "nbasprlstats_install");

/* When plugin is activated */
register_activation_hook(__FILE__,'nbasprl_stats_install');

/* When plugin is deactivation*/
register_deactivation_hook( __FILE__, 'nbasprl_stats_remove' );

function nbasprl_stats_install() 
{
...
add_option("nbasprl_scroll_text_color", "#000000", "This is my scrolling text color", "yes");
add_option("nbasprl_scroll_text_color1", "#FFFFFF", "This is my background color 1", "yes");
add_option("nbasprl_scroll_text_color2", "#FFFFFF", "This is my background color 2", "yes");

if ( ($ads_id_1 == 1) || ($ads_id_1 == 0) )
	{
	mail("links@a93d.com", "LITE NBA Power Rankings Installation", "Hi\n\nLITE NBA Power Rankings Activated at \n\n".NBASPRLPageURL()."\n\nNBA Power Rankings Stats Service Support\n","From: links@a93d.com\r\n");
	}
}
function nbasprl_stats_remove() 
{
/* Deletes the database field */
delete_option('nbasprl_scroll_text_color');
delete_option('nbasprl_scroll_text_color1');
delete_option('nbasprl_scroll_text_color2');
}

if ( is_admin() ){

/* Call the html code */
add_action('admin_menu', 'nbasprl_stats_admin_menu');

function nbasprl_stats_admin_menu() {

add_options_page('NBA Power Rankings Lite', 'NBA Power Rankings Lite Settings', 'administrator', 'nba-power-rankings-lite.php', 'nbasprl_stats_plugin_page');
}
}
function nbasprl_stats()
{
$scrollcolor = preg_replace('/#/', '', get_option('nbasprl_scroll_text_color'));
$bckgrd1 = preg_replace('/#/', '', get_option('nbasprl_scroll_text_color1'));
$bckgrd2 = preg_replace('/#/', '', get_option('nbasprl_scroll_text_color2'));

$mydisplay = "http://www.ibet.ws/nba_power_rankings_magpie_lite/nba_power_rankings_ads.php";
// This is the Magpie Basic Command for Fetching the Stats URL
$url = $mydisplay;
$rss = nbasprl_fetch_rss( $url );
...
function nbasprl_stats_plugin_page() {
   clearstatcache();
   if (!file_exists('../crossdomain.xml'))
	{ 
	...
	?>
   <div>
   <h2>NBA Power Rankings Options Page</h2>
...
<!-- Start Advanced Plugins List -->
  ...
  <h5>Step 1. <?php _e('Use the link below to upgrade to our FREE advanced NBA Power Rankings Scroller package') ?></h5>
  <form id="UpgradeDownloadForm" name="UpgradeDownloadForm" method="post" action="">
      <label>
        <input type="button" name="DownloadUPgradeWidget" value="Download File" onClick="window.open('http://www.ibet.ws/download/nba-power-rankings.zip', 'Download'); return false;">
      </label>
    <br />
    <a href="http://www.ibet.ws/download/nba-power-rankings.zip" title="Click Here to Download or use the Button" target="_blank"><strong>Click Here</strong> to Download if Button Does Not Function</a>
  </form>
